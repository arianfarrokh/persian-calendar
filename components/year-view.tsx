...
function MonthCalendar({
  ...
  const firstDayWeekday = getDay(monthStart); // 0=یکشنبه ... 6=شنبه
  ...
  // سلول های ماه
  const dayCells = [
    ...Array.from({ length: firstDayWeekday }).map((_, i) => <Box key={`empty-${i}`} />),
    ...Array.from({ length: daysInMonth }).map((_, i) => {
      ...
      return (
        <Box display={'grid'} gridTemplateColumns={'repeat(7,1fr)'} key={day}>
  ...
  return (
    <Card variant="outlined" sx={{ p: 1, textAlign: "center" }}>
      <CardHeader
        ...
        sx={{ pb: 1 }}
      />
      <Box  display={'grid'} gridTemplateColumns={'repeat(7,1fr)'} justifyContent="center">
...
export function YearView({ selectedDate, events, onDateSelect, onCreateEvent }: YearViewProps) {
  ...
  const yearStats = useMemo(() => {
    ...
    const eventsByMonth = PERSIAN_MONTHS.map((_, i) => {
      const month = i + 1;
      return yearEvents.filter((e) => getMonth(e.startDate) + 1 === month).length;
    });
    return {
    ...
    <Stack spacing={2}>
      <Card variant="outlined" sx={{ p: 2 }}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Box>
            <Typography variant="h5">سال {toPersianNumbers(year.toString())}</Typography>
